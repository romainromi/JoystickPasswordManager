...
export const createUser = async (email, passwordHash, verifyToken) => {
	const [result] = await db.query(`INSERT INTO utilisateurs (email, password, verify_token) VALUES (?, ?, ?)`, [email, passwordHash, verifyToken]);
...
export const saveResetPassword = async (userId, token) => {
	await db.query(`UPDATE utilisateurs SET reset_token = ? WHERE id = ?`, [token, userId]);
};
